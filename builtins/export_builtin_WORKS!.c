...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   export_builtin_FIXED_ISSUE_97.c                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ccarrace <ccarrace@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/13 21:00:11 by ccarrace          #+#    #+#             */
/*   Updated: 2023/12/01 00:06:14 by ccarrace         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int set_new_var(t_env_lst **head, char *arg)
{
    ...
    if (new_node == NULL)
        return (1);
printf("equal sign position = %zu | %zu = arg_len\n", equal_sign_position, arg_len);
    if (equal_sign_position == 0)
	{
printf("%s has no equal sign\n", arg);
        new_node->var_name = ft_strdup(arg);
        new_node->var_value = NULL;
    }
	else
	{
printf("%s includes an equal\n", arg);
